...
export async function run(options) {
	try {
		...
		if (options.type.includes('metaobject')) {
			...
			while (true) {
				...
				if (options.type === 'metaobject_definitions') {
					result = [
						...
						...data.nodes
							.map(item =>
								item.fieldDefinitions.map(itemField => ({
									id: item.id,
									name: item.name,
									type: item.type,
					...
					const entryData = await Promise.all(
						data.nodes.map(async definition => {
							let entryEndCursor = '';
							const entries = await shopifyGraphMetaobjectEntries({
								store: options.url,
							...
							bar1.increment(1);
							if (
								options.fields.includes('fieldValue-field') ||
								options.fields.includes('fieldKey-field')
							)
								result = [
									...result,
									...(entries
										? entries.nodes
												.map(item =>
													item.fields.map((itemField, index) => ({
		...
		} else if (options.type.includes('metafield')) {
			let endCursor = '';
			bar1.start(0, 0);

			while (true) {
				const data = await shopifyGraphMetafield({ store: options.url, endCursor });
			...
			while (true) {
				...
				if (
					(options.type === 'custom_collections' && filters.products?.length) ||
					options.type === 'price_rules' ||
					options.type === 'blogs'
				) {
									...
									if (isMain(options.type, filter)) {
											...
											for (let i = 0; i < maxLength; i++) {
												...
												if (
													...
													['string', 'number'].includes(
														typeof item[filter][i],
													) &&
													filter.includes('ids')
												) {
													if (filter.includes('product_ids')) {
														const itemValue = await shopifyItemRESTApi(
															options.url,
															'products',
															item[filter][i],
															'get',
										...
										temp = {
											...
											[filter]: Array.isArray(temp[filter])
												...
												: JSON.stringify(
														_.pick(temp[filter], filters[filter]),
												  ),
										};
									}
								}
								...
								for (let i = 0; i < temp.length; i++) {
									if (functions.basic?.length) {
										for (const func of functions.basic) {
											...
											if (param) {
												temp[i] = convertion[options.type]['basic'][
													funcName
												](temp[i], param);
											} else {
												temp[i] = convertion[options.type]['basic'][
								...
								if (functions.basic?.length) {
									for (const func of functions.basic) {
										...
										if (param) {
											...
											temp =
												convertion[options.type]['basic'][funcName](temp);
										}
									}
								}
							}
						...
						data[options.type].map(async item => {
							...
							for (const filter in filters) {
								if (
									item[filter]?.length ||
									(options.type === 'orders' &&
										orderMultiFields().includes(filter))
								) {
									...
									if (isMain(options.type, filter)) {
										...
										if (!Array.isArray(temp))
											...
											if (item[filter].length > 0)
												for (let i = 0; i < tempLength; i++) {
													temp.push({
														client_details__line_type_command:
															capitalize(filter).slice(0, -1),
														line_items__line_command: 'DEFAULT',
														line_items__force_gift_card: 'No',
														..._.omit(
															itemWithFunc,
										...
										temp = {
											...
											[filter]: Array.isArray(item[filter])
												...
												: JSON.stringify(
														_.pick(item[filter], filters[filter]),
												  ),
										};
									}
